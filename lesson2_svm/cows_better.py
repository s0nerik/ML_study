...
FEATURE_COLUMN_NAMES = ["d7", "d6", "d5", "d4", "d3", "d2", "d1", "health"]
# FEATURE_COLUMN_NAMES = ["d7", "d6", "d5", "d4", "d3", "d2", "d1"]
...
# Get input data
data = pd.read_csv('input/cowhealth_normalized.csv', header=0, usecols=FEATURE_COLUMN_NAMES)
train_data = data[::2]
test_data = data[1::2]
# train_data.head()
...
# Learn!
estimator.fit(input_fn=input_train, steps=50)
...
def input_test():
    features = {
        k: tf.constant(test_data[k].values)
        # k: tf.constant([test_data[k].values[0], train_data[k].values[1]])
        for k in FEATURE_COLUMN_NAMES[:-1]
    }
    return features
